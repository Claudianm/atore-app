// ALMACEN APP CON SUPABASE AUTH - v5
...
const CATEGORIAS = ["Todos", "Alimentos", "Bebidas", "Limpieza", "Lácteos", "Otro"];
...
function stockTotal(p) { return p.marcas.reduce((a, m) => a + m.stock, 0); }
function minimoTotal(p) { return p.marcas.reduce((a, m) => a + m.minimo, 0); }
...
// ============ DATOS INICIALES ============
const invInicial = [
  { id: 1, nombre: "Fideos", categoria: "Alimentos", marcas: [
    { id: 11, marca: "Carozzi 500g",  precioCompra: 420, precioVenta: 650,  stock: 22, minimo: 10, vendidos: 18 },
    { id: 12, marca: "Luchetti 500g", precioCompra: 490, precioVenta: 720,  stock: 9,  minimo: 5,  vendidos: 9  },
    { id: 13, marca: "Súper 500g",    precioCompra: 280, precioVenta: 480,  stock: 24, minimo: 8,  vendidos: 24 },
  ]},
  { id: 2, nombre: "Arroz", categoria: "Alimentos", marcas: [
    { id: 21, marca: "Tucapel 1kg",   precioCompra: 750, precioVenta: 1200, stock: 20, minimo: 8,  vendidos: 30 },
    { id: 22, marca: "Gallo 1kg",     precioCompra: 820, precioVenta: 1350, stock: 18, minimo: 6,  vendidos: 15 },
  ]},
  { id: 3, nombre: "Aceite", categoria: "Alimentos", marcas: [
    { id: 31, marca: "Chef 900ml",    precioCompra: 1800, precioVenta: 2800, stock: 5, minimo: 8,  vendidos: 12 },
    { id: 32, marca: "Miraflores 1L", precioCompra: 2100, precioVenta: 3200, stock: 3, minimo: 5,  vendidos: 8  },
  ]},
  { id: 4, nombre: "Leche", categoria: "Lácteos", marcas: [
    { id: 41, marca: "Colún 1L",      precioCompra: 700,  precioVenta: 1100, stock: 12, minimo: 6, vendidos: 40 },
    { id: 42, marca: "Soprole 1L",    precioCompra: 720,  precioVenta: 1150, stock: 8,  minimo: 6, vendidos: 22 },
  ]},
  { id: 5, nombre: "Detergente", categoria: "Limpieza", marcas: [
    { id: 51, marca: "Omo 500g",      precioCompra: 1100, precioVenta: 1800, stock: 3,  minimo: 5, vendidos: 10 },
    { id: 52, marca: "Drive 500g",    precioCompra: 900,  precioVenta: 1500, stock: 6,  minimo: 4, vendidos: 18 },
  ]},
];
const pagosInicial = [
  { id: 1, tipo: "Venta",     descripcion: "Venta del día",       monto: 32400, fecha: "2026-05-14", pagado: true  },
  { id: 2, tipo: "Proveedor", descripcion: "Distribuidora Norte", monto: 48000, fecha: "2026-05-14", pagado: true  },
  { id: 3, tipo: "Fiado",     descripcion: "Juan Torres",         monto: 8200,  fecha: "2026-05-10", pagado: false },
  { id: 4, tipo: "Gasto",     descripcion: "Luz del local",       monto: 12000, fecha: "2026-05-12", pagado: true  },
  { id: 5, tipo: "Venta",     descripcion: "Venta del día",       monto: 28900, fecha: "2026-05-13", pagado: true  },
  { id: 6, tipo: "Fiado",     descripcion: "María López",         monto: 5500,  fecha: "2026-05-11", pagado: false },
  { id: 7, tipo: "Venta",     descripcion: "Venta del día",       monto: 41200, fecha: "2026-05-15", pagado: true  },
  { id: 8, tipo: "Gasto",     descripcion: "Bolsas y empaques",   monto: 3200,  fecha: "2026-05-11", pagado: true  },
];
const pedidosInicial = [
  { id: 1, proveedor: "Distribuidora Norte", fecha: "2026-05-13", estado: "En camino", productos: ["Arroz","Fideos","Azúcar","Aceite"], monto: 84000 },
  { id: 2, proveedor: "Lácteos El Campo",    fecha: "2026-05-10", estado: "Recibido",  productos: ["Leche","Yogur","Mantequilla"],    monto: 42000 },
  { id: 3, proveedor: "Limpieza Total",      fecha: "2026-05-16", estado: "Pendiente", productos: ["Detergente","Cloro","Esponja"],   monto: 18200 },
];
const recInicial = [
  { id: 1, tipo: "Fiado",  titulo: "Cobrar a Juan Torres",      descripcion: "Debe $8.200 desde el 10 de mayo", fecha: "2026-05-18", prioridad: "Alta",  completado: false },
  { id: 2, tipo: "Pago",   titulo: "Pagar luz del local",       descripcion: "Vence el 20 de mayo",             fecha: "2026-05-20", prioridad: "Alta",  completado: false },
  { id: 3, tipo: "Pedido", titulo: "Llamar a Distribuidora",    descripcion: "Confirmar entrega del pedido",    fecha: "2026-05-17", prioridad: "Media", completado: false },
  { id: 4, tipo: "Fiado",  titulo: "Cobrar a María López",      descripcion: "Debe $5.500 desde el 11 de mayo", fecha: "2026-05-19", prioridad: "Media", completado: false },
  { id: 5, tipo: "Otro",   titulo: "Renovar permiso municipal", descripcion: "Vence a fin de mes",              fecha: "2026-05-30", prioridad: "Baja",  completado: false },
];
const ajustesInicial = [
  { id: 1, categoria: "ingreso", descripcion: "Cobro fiado Juan Torres", monto: 8200  },
  { id: 2, categoria: "gasto",   descripcion: "Arriendo local",          monto: 85000 },
  { id: 3, categoria: "gasto",   descripcion: "Sueldo ayudante",         monto: 60000 },
];
...
function SelectorBotones({ opciones, activo, onChange, iconos }) {
  return (
    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
      {opciones.map(o => (
        <button key={o} onClick={() => onChange(o)} style={{ padding: "6px 12px", borderRadius: 20, fontSize: 12, cursor: "pointer", border: "0.5px solid", transition: "all 0.15s", background: activo === o ? "#1a3a2a" : "var(--color-background-secondary,#f5f4f0)", color: activo === o ? "white" : "var(--color-text-primary,#111)", borderColor: activo === o ? "#1a3a2a" : "var(--color-border-tertiary,#ddd)" }}>
...
function Filtros({ opciones, activo, onChange, iconos }) {
  return (
    <div style={{ display: "flex", gap: 6, padding: "10px 14px", overflowX: "auto", scrollbarWidth: "none" }}>
      {opciones.map(o => (
        <button key={o} onClick={() => onChange(o)} style={{ padding: "5px 12px", borderRadius: 20, fontSize: 12, whiteSpace: "nowrap", cursor: "pointer", border: "0.5px solid", transition: "all 0.15s", background: activo === o ? "#1a3a2a" : "var(--color-background-primary,#fff)", color: activo === o ? "white" : "var(--color-text-secondary,#888)", borderColor: activo === o ? "#1a3a2a" : "var(--color-border-tertiary,#ddd)" }}>
...
// ============ INVENTARIO ============
function ModalMarca({ marca, onGuardar, onCerrar }) {
  const [form, setForm] = useState(marca || { marca: "", precioCompra: "", precioVenta: "", stock: "", minimo: "", vendidos: 0 });
  const set = (k, v) => setForm(f => ({ ...f, [k]: v }));
  const valido = form.marca.trim() && form.precioCompra !== "" && form.precioVenta !== "" && form.stock !== "" && form.minimo !== "";
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 200 }}>
      <div style={{ background: "var(--color-background-primary,#fff)", borderRadius: "16px 16px 0 0", padding: "24px 20px 36px", width: "100%", maxWidth: 420, maxHeight: "88vh", overflowY: "auto" }}>
        ...
        {[
          { label: "Marca / Descripción",   key: "marca",        type: "text",   placeholder: "Ej: Carozzi 500g" },
...
function DetalleProducto({ producto, onVolver, onActualizar }) {
  ...
  const totalVendidos = producto.marcas.reduce((a, m) => a + (m.vendidos || 0), 0);
  const margenProm = producto.marcas.length > 0 ? Math.round(producto.marcas.reduce((a, m) => a + margenPct(m), 0) / producto.marcas.length) : 0;
  ...
  return (
    <div>
      ...
      {modalMarca && <ModalMarca marca={modalMarca === "nuevo" ? null : modalMarca} onGuardar={guardarMarca} onCerrar={() => setModalMarca(null)} />}
...
function Inventario({ data, setData }) {
  ...
  const filtrados = data.filter(p => p.nombre.toLowerCase().includes(busqueda.toLowerCase()) && (categoria === "Todos" || p.categoria === categoria));
  ...
  return (
    <>
      <div style={{ background: "#1a3a2a", padding: "16px 16px 14px" }}>
        <div style={{ color: "#7fbb95", fontSize: 11, marginBottom: 2 }}>Inventario</div>
      ...
      <div style={{ padding: "10px 14px 0" }}><input style={{ ...inputStyle, background: "var(--color-background-primary,#fff)" }} type="text" placeholder="Buscar producto..." value={busqueda} onChange={e => setBusqueda(e.target.value)} /></div>
      <Filtros opciones={CATEGORIAS} activo={categoria} onChange={setCategoria} />
      <div style={{ padding: "0 14px", paddingBottom: 8 }}>
        {filtrados.length === 0 && <div style={{ textAlign: "center", padding: "32px 0", color: "var(--color-text-secondary,#888)", fontSize: 14 }}>No se encontraron productos</div>}
        {filtrados.map(p => {
          ...
          const barColor = est === "sin" ? "#e24b4a" : est === "poco" ? "#ef9f27" : "#1d9e75";
          const mp = p.marcas.length > 0 ? Math.round(p.marcas.reduce((a, m) => a + margenPct(m), 0) / p.marcas.length) : null;
          ...
          return (
            <div key={p.id} onClick={() => setDetalle(p)} style={{ background: "var(--color-background-primary,#fff)", border: "0.5px solid var(--color-border-tertiary,#ddd)", borderRadius: 12, padding: "12px 14px", marginBottom: 8, cursor: "pointer" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div style={{ flex: 1 }}><div style={{ fontWeight: 500, fontSize: 14 }}>{p.nombre}</div><div style={{ fontSize: 12, color: "var(--color-text-secondary,#888)", marginTop: 2 }}>{p.categoria} · {p.marcas.length} marca{p.marcas.length !== 1 ? "s" : ""}{tv > 0 && ` · ${tv} vendidos`}</div></div>
      ...
      {modal && (
        <ModalBase titulo={modal === "nuevo" ? "Nuevo producto" : "Editar producto"} onCerrar={() => setModal(null)} onGuardar={() => guardar(modal === "nuevo" ? { nombre: modal._n, categoria: modal._c || "Alimentos" } : modal)} valido={true} labelGuardar={modal === "nuevo" ? "Crear producto" : "Guardar"}>
          {(() => {
            const [n, setN] = useState(modal === "nuevo" ? "" : modal.nombre);
            const [c, setC] = useState(modal === "nuevo" ? "Alimentos" : modal.categoria);
            useEffect(() => { if (modal !== "nuevo") { /* no-op */ } }, []);
            return (
              <>
                <Campo label="Nombre del producto"><input style={inputStyle} type="text" value={n} placeholder="Ej: Fideos, Arroz, Aceite..." onChange={e => setN(e.target.value)} /></Campo>
                <Campo label="Categoría"><select style={inputStyle} value={c} onChange={e => setC(e.target.value)}>{CATEGORIAS.filter(x => x !== "Todos").map(x => <option key={x}>{x}</option>)}</select></Campo>
...
function Pagos({ data, setData }) {
  ...
  const filtrados = data.filter(p => filtro === "Todos" || p.tipo === filtro);
  ...
  return (
    <>
      <div style={{ background: "#1a3a2a", padding: "16px 16px 14px" }}>
        <div style={{ color: "#7fbb95", fontSize: 11, marginBottom: 2 }}>Pagos y gastos</div>
      ...
      {tab === "lista" && (
        <>
          ...
          <div style={{ padding: "0 14px", paddingBottom: 8 }}>
            {filtrados.length === 0 && <div style={{ textAlign: "center", padding: "32px 0", color: "var(--color-text-secondary,#888)", fontSize: 14 }}>Sin registros</div>}
            {filtrados.map(p => (
    ...
    <ModalBase titulo={pedido ? "Editar pedido" : "Nuevo pedido"} onCerrar={onCerrar} onGuardar={() => onGuardar(form)} valido={valido} labelGuardar={pedido ? "Guardar cambios" : "Crear pedido"}>
      ...
      <Campo label="Productos">
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
          ...
          {form.productos.length === 0 && <span style={{ fontSize: 12, color: "var(--color-text-secondary,#aaa)" }}>Agrega al menos un producto</span>}
        </div>
      </Campo>
    </ModalBase>
  );
...
function Pedidos({ data, setData }) {
  ...
  return (
    <>
      ...
      <div style={{ padding: "0 14px", paddingBottom: 8 }}>
        {filtrados.length === 0 && <div style={{ textAlign: "center", padding: "32px 0", color: "var(--color-text-secondary,#888)", fontSize: 14 }}>Sin pedidos</div>}
        {filtrados.map(p => (
          <div key={p.id} style={{ background: "var(--color-background-primary,#fff)", border: "0.5px solid var(--color-border-tertiary,#ddd)", borderRadius: 12, padding: "12px 14px", marginBottom: 8 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
...
function ModalRecordatorio({ rec, onGuardar, onCerrar }) {
  ...
  return (
    <ModalBase titulo={rec ? "Editar recordatorio" : "Nuevo recordatorio"} onCerrar={onCerrar} onGuardar={() => onGuardar(form)} valido={valido} labelGuardar={rec ? "Guardar cambios" : "Crear recordatorio"}>
      ...
      <Campo label="Prioridad"><div style={{ display: "flex", gap: 6 }}>{PRIORIDADES.map(p => (<button key={p} onClick={() => set("prioridad", p)} style={{ flex: 1, padding: "7px", borderRadius: 8, fontSize: 13, cursor: "pointer", border: "0.5px solid", background: form.prioridad === p ? "#1a3a2a" : "var(--color-background-secondary,#f5f4f0)", color: form.prioridad === p ? "white" : "var(--color-text-primary,#111)", borderColor: form.prioridad === p ? "#1a3a2a" : "var(--color-border-tertiary,#ddd)" }}>{p}</button>))}</div></Campo>
    </ModalBase>
  );
}
function Recordatorios({ data, setData }) {
  ...
  return (
    <>
      <div style={{ background: "#1a3a2a", padding: "16px 16px 14px" }}>
        <div style={{ color: "#7fbb95", fontSize: 11, marginBottom: 2 }}>Recordatorios</div>
      ...
      <div style={{ padding: "0 14px", paddingBottom: 8 }}>
        {pendientes.length === 0 && <div style={{ textAlign: "center", padding: "32px 0", color: "var(--color-text-secondary,#888)", fontSize: 14 }}>🎉 Sin recordatorios pendientes</div>}
        {pendientes.map(r => {
      ...
      <FAB label="+ Recordatorio" onClick={() => setModal("nuevo")} />
    ...
    <ModalBase titulo={ajuste ? "Editar ajuste" : "Nuevo ajuste manual"} onCerrar={onCerrar} onGuardar={() => onGuardar(form)} valido={valido} labelGuardar={ajuste ? "Guardar cambios" : "Agregar ajuste"}>
      <Campo label="Tipo">
        <div style={{ display: "flex", gap: 8 }}>
          {["ingreso", "gasto"].map(c => (<button key={c} onClick={() => set("categoria", c)} style={{ flex: 1, padding: "9px", borderRadius: 8, fontSize: 13, cursor: "pointer", border: "0.5px solid", background: form.categoria === c ? (c === "ingreso" ? "#1d9e75" : "#a32d2d") : "var(--color-background-secondary,#f5f4f0)", color: form.categoria === c ? "white" : "var(--color-text-primary,#111)", borderColor: form.categoria === c ? "transparent" : "var(--color-border-tertiary,#ddd)" }}>{c === "ingreso" ? "➕ Ingreso" : "➖ Gasto"}</button>))}
        </div>
      </Campo>
    ...
    <>
      <div style={{ background: "#1a3a2a", padding: "16px 16px 18px" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-end", gap: 10 }}>
          ...
          <div style={{ background: "rgba(255,255,255,0.08)", borderRadius: 8, padding: "6px 12px", marginBottom: 2 }}><div style={{ fontSize: 10, color: "#7fbb95" }}>Margen</div><div style={{ fontSize: 18, fontWeight: 700, color: utilidadNeta >= 0 ? "#7febb8" : "#f28b7a" }}>{margen}%</div></div>
        </div>
...
export default function AlmacenApp() {
  ...
  const [inventario, setInventario] = useState(() => { try { const s = localStorage.getItem("inv_v4"); return s ? JSON.parse(s) : invInicial; } catch { return invInicial; } });
  const [pagos, setPagos]           = useState(() => { try { const s = localStorage.getItem("pag_v4"); return s ? JSON.parse(s) : pagosInicial; } catch { return pagosInicial; } });
  const [pedidos, setPedidos]       = useState(() => { try { const s = localStorage.getItem("ped_v4"); return s ? JSON.parse(s) : pedidosInicial; } catch { return pedidosInicial; } });
  const [recs, setRecs]             = useState(() => { try { const s = localStorage.getItem("rec_v4"); return s ? JSON.parse(s) : recInicial; } catch { return recInicial; } });
  const [ajustes, setAjustes]       = useState(() => { try { const s = localStorage.getItem("aj_v4");  return s ? JSON.parse(s) : ajustesInicial; } catch { return ajustesInicial; } });
